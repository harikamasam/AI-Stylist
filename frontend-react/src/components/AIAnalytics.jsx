...
function AIAnalytics({ category, style }) {
  const { analytics, averageScore, loading, error } = useAnalytics(
    category,
    style
  );

  return (
    <section className="overflow-hidden rounded-[28px] border border-white/10 bg-zinc-950/90 p-4 shadow-2xl shadow-black/30 backdrop-blur sm:p-5 lg:p-6">
      <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-4 py-3 text-left sm:text-right">
          <p className="text-gray-400 text-xs font-bold uppercase">Overall</p>
      ...
      {error ? (
        <div className="bg-rose-500/10 border border-rose-400/20 text-rose-200 rounded-2xl p-4 font-bold">
          {error}
        </div>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2">
          {ANALYTICS_METRICS.map((metric) => {
            ...
            return (
              <div
                key={metric.key}
                className="rounded-2xl border border-white/10 bg-zinc-900/80 p-3 transition duration-300 hover:-translate-y-0.5 hover:border-cyan-300/40 sm:p-4"
              >
